...
def test_tki_ai_model_local(
    ...
    headfile = tmp_path / "GWF_1" / "HEAD.HED"
    grbfile = tmp_path / "GWF_1" / "MODELINPUT" / "T-MODEL-F.DIS6.grb"
    heads = imod.mf6.open_hds(headfile, grbfile, False)
    starttime = pd.to_datetime("2000/01/01")
    timedelta = pd.to_timedelta(heads["time"], "D")
    heads.assign_coords(time=starttime + timedelta)
    imod.idf.save(tmp_path / "GWF_1" / "head.idf", heads)
