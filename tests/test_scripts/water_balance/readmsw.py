...
def totfile2df(msw_totfile):
    ...
    for colname in list(msw_totdf):
        if re.match(r"decS\d{2}",colname):
            msw_totdf['decS_in'] = msw_totdf['decS_in'] + [max(colvalue,0.0) for colvalue in np.array(msw_totdf[colname])]
            msw_totdf['decS_out'] = msw_totdf['decS_out'] + [min(colvalue,0.0) for colvalue in np.array(msw_totdf[colname])]
