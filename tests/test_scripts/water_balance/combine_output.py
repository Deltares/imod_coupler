...
#   timevar[:] = np.array(df.index)
    if singlevar:
        ...
        xchgvar = ds.createVariable("exchange","f8",("time","id",))
        namevar = ds.createVariable("varname","S1",("id","nchar",))
        xchgvar[:] = df.values
...
def combineDF(**kwargs):
    ...
    for key in ['decSic','decSpdmac','decSpdmic','Pm','Psgw','Pssw','qrun','qdr','qmodf']:
    ...
    for key in ['decSic','decSpdmac','decSpdmic','Esp','Eic','Epd','Ebs','Tact','qrun','qdr','qspgw','qmodf']:
        tmp = msw_totdf[key].values
        tmp[tmp>0.0] = 0.0
    ...
    combined['t'] = combined['t'] / 86400.0   # time was the fm time in seconds, now converted into days
    return combined
...
if __name__ == '__main__':
    ...
    sys.stderr.write('imported, use function CombineDF(fm_hisfile, msw_totfile, mf_listfile) to obtain the combined dataframe.')
