...
def test_ribamod_bucket(
    ...
    basin_df = pd.read_feather(
        tmp_path_dev / ribametamod_model._ribasim_model_dir / "output" / "basin.arrow"
    )
...
def test_ribametamod_backwater(
    ...
    flow_df = pd.read_feather(
        tmp_path_dev / ribametamod_model._ribasim_model_dir / "output" / "flow.arrow"
    )
    ...
    budgets = imod.mf6.open_cbc(
        tmp_path_dev / ribametamod_model._modflow6_model_dir / "GWF_1" / "GWF_1.cbc",
        tmp_path_dev / ribametamod_model._modflow6_model_dir / "GWF_1" / "dis.dis.grb",
    )
