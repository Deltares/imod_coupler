import numpy as np
import pandas as pd
from test_utilities import compute_tolerance_per_column_dataframe
...
def test_compare_absolute_fail():
    ...
    assert not compute_tolerance_per_column_dataframe(
        frame1, frame2, abstol=0.009, reltol=100
    )
...
def test_compare_absolute_succeed():
    ...
    assert compute_tolerance_per_column_dataframe(
        frame1, frame2, abstol=0.011, reltol=100
    )
...
def test_compare_relative_fail():
    ...
    frame1 = pd.DataFrame([[10, -10], [1, -1]])
    assert not compute_tolerance_per_column_dataframe(
        frame1, frame2, abstol=100, reltol=0.001
    )
...
def test_compare_relative_succeed():
    ...
    assert compute_tolerance_per_column_dataframe(
        frame1, frame2, abstol=100, reltol=0.011
    )
