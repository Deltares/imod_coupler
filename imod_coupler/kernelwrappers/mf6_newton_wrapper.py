...
class ExpandArray:
    ...
    from internal size to user size and 3D shape (layer, row, col). This is
    ...
    def inflate_3d(self) -> NDArray[Any]:
        """
        Puts internal(reduced) array values in user-defined array,
        and returns 3D-vieuw
        """
        return self.inflate().reshape((self.nlay, self.nrow, self.ncol))
...
class PhreaticModelArray:
    ...
    Parameters
    ----------
    shape: tuple[int, int, int]:     list that contains the grid demensions layer, row and column
    ...
    def set_ptr(self, new_values: NDArray[Any]) -> None:
        ...
        """
        self.variable.reduced[self.phreatic_reduced_index] = new_values
    ...
    def get_ptr(self) -> Any:
        ...
        """
        return self.variable.inflate_3d()[
            self.phreatic_layer_user_index,
            self.row_user_index,
            self.col_user_index,
        ].flatten()  # type ignore

    @property
    def phreatic_layer_user_index(self) -> Any:
        # TODO: use max layer from input in case of dry columns
        self._ensure_user_indices()
        phreatic_layer = np.argmax(self.saturation.inflate_3d() > 0, axis=0).flatten()[
            self.nodes
        ]
        return np.minimum(phreatic_layer, self.max_layer)

    @property
    def phreatic_reduced_index(self) -> Any:
...
class PhreaticBCArray(PhreaticModelArray):
    ...
    def set_ptr_nodes(self) -> None:
        self.variable.nodelist[:] = (self.phreatic_reduced_index + 1)[:]  # type ignore
