...
import IMODCollector.buildTypes.IMODCollector_X64Release52
...
object TestbenchCouplerWin64Release52 : BuildType({
    name = "Testbench Coupler Win64 Release 5.2"
    ...
    params {
        param("env.IMODC__KERNELS__METASWAP__DLL", """%teamcity.build.checkoutDir%\dll\MetaSWAP.dll""")
        param("env.PATH", """%system.teamcity.build.workingDir%\imod_coupler;%env.PATH%""")
        param("copy_cases", "no")
        param("env.IMODC__KERNELS__METASWAP__DLL_DEPENDENCY", """%teamcity.build.checkoutDir%\dll""")
        param("env.IMODC__KERNELS__MODFLOW6__DLL", """%teamcity.build.checkoutDir%\dll\libmf6.dll""")
    }
    ...
    steps {
        ...
        exec {
            ...
            arguments = """TestBench.py --username %svn_buildserver_username% --password %svn_buildserver_password% --compare --config configs\ModMf6Coupler\ModMf6Coupler_win64_iMOD-5.2.xml --teamcity"""
    ...
    triggers {
        vcs {
            enabled = false
            triggerRules = """
                +:root=iMOD6_Coupler_Imod6coupler:**
                +:root=iMOD6_Coupler_GitXmipy:**
            """.trimIndent()

        }
        finishBuildTrigger {
            enabled = false
            buildType = "${IMODCollector_X64Release52.id}"
            successfulOnly = true
        }
    }
    ...
    dependencies {
        dependency(IMODCollector.buildTypes.IMODCollector_X64Release52) {
            snapshot {
            }
